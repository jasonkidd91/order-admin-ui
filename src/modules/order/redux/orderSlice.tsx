...
import { OrderState, ScreenState } from './types';
...
export const updateOrder = (auth: any, payload: any) => {
  // return (dispatch: any) => {
  return new Api(auth).updateOrder(payload).then((res) => {
    if (res.data) {
      const { errors, data } = res.data;
      if (errors) {
        console.log('order::updateOrder', errors);
        let errorMessage = errors instanceof Array ? errors[0] : errors;
        throw new Error(errorMessage);
      } else {
        return data;
