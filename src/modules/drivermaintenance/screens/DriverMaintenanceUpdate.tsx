...
import { getDriverData, updateDriver } from '../redux/driverSlice';
import { useParams } from 'react-router-dom';

const DriverMaintenanceUpdate = () => {
  ...
  useEffect(() => {
    getDriverData(auth, id).then((data: any) => {
      setDriverState(data);
    });
  }, []);

  const updateDriverHandler = () => {
    dispatch(updateDriver(auth, driverState, id));
  };
          ...
          <CFormGroup row className="my-0">
            <CCol xs="6">
              <CFormGroup>
                ...
                />
                <CSelect value={driverState.flag_colour} onChange={(ev) => handleAssignFlag(ev)}>
          ...
          <CRow>
            <CCol xs="12">
              <div className="d-flex flex-row-reverse">
                ...
                <CButton color="success" className="px-4 mr-4" onClick={updateDriverHandler}>
                  Update
                </CButton>
              </div>
            </CCol>
...
export default DriverMaintenanceUpdate;
