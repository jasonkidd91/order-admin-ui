...
import { createPickup, getStoreDetails } from '../redux/pickupSlice';
...
const PickupCreate = () => {
  ...
  const createPickupHandler = () => {
    dispatch(createPickup(auth, pickupState));
  };
  ...
  useEffect(() => {
    getStoreDetails(auth, store_id).then((data: any) => {
      setStoreState(data);
    });
  }, []);
        ...
        <CCardBody>
          ...
          <CRow>
            <CCol xs={12} className="text-right">
              <CButton color="success" className="px-4 mr-4" onClick={createPickupHandler}>
                Create
              </CButton>
